...
export default function FacultyLayout({ children }: { children: React.ReactNode }) {
  ...
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
    ...
    const [facultyEmail, setFacultyEmail] = useState('Loading...');
    ...
    useEffect(() => {
      const fetchUserProfile = async () => {
        ...
        if (user) {
          setFacultyEmail(user.email ?? 'Email not available');
          ...
          if (profileData) {
            setFacultyName(`${profileData.name}`);
          }
        } else {
          router.push('/login'); 
        }
      ...
      >
        <div className="w-72 flex flex-col h-full bg-slate-900">
          <div className="p-6 border-b border-slate-800 flex flex-col items-center text-center">
            ...
            <h2 className="font-bold text-lg text-white">{facultyName}</h2>
            <p className="text-xs text-slate-400 mt-1">{facultyEmail}</p>
